...
def format_record(record: dict[str, any]) -> str:
    ...
    if record["extra"]:
        extra_str = ", ".join([f"{key}={value}" for key, value in record["extra"].items()])
...
class InterceptHandler(logging.Handler):
    def emit(self, record):
        ...
        logger.patch(lambda r: r.update(name=record.name, function=record.funcName, line=record.lineno)).log(level, record.getMessage())
